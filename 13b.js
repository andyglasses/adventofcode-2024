function run(){
    ...
    for(var l = 0; l < input.length; l++){
        var buttonA = parseButton(input[l], 3);
        l++;
        var buttonB = parseButton(input[l], 1);
        l++;
        var prize = parsePrize(input[l]);
    ...
    machines.forEach((m, i) =>{
       /* if(((100 * m.buttonA.x) + (100 * m.buttonB.x)) < m.prize.x){
            console.log(i +1, 'cant reach x');
            return;
        }

        if(((100 * m.buttonA.y) + (100 * m.buttonB.y)) < m.prize.y){
            console.log(i +1, 'cant reach y');
            return;
        }*/

        var a = Math.floor(m.prize.x / m.buttonA.x);
        var b = 0;
        /*if(a > maxButtons){
            a = maxButtons;
        }*/
        while(a > 0 && (m.prize.x !== (a * m.buttonA.x) + (b * m.buttonB.x) || m.prize.y !== (a * m.buttonA.y) + (b * m.buttonB.y))){
            if(m.prize.x > ((a * m.buttonA.x) + (b * m.buttonB.x))){
                b+= Math.ceil((m.prize.x - ((a * m.buttonA.x) + (b * m.buttonB.x))) / m.buttonB.x)
            }
            if(m.prize.y > ((a * m.buttonA.y) + (b * m.buttonB.y))){
                b+= Math.ceil(( m.prize.y - ((a * m.buttonA.y) + (b * m.buttonB.y))) / m.buttonB.y)
            }
            if(a > 0 && m.prize.x < ((a * m.buttonA.x) + (b * m.buttonB.x))){
                a-= Math.ceil(((a * m.buttonA.x) + (b * m.buttonB.x) - m.prize.x) / m.buttonA.x)
            } 
            if(a > 0 && m.prize.y < ((a * m.buttonA.y) + (b * m.buttonB.y))){
                a-= Math.ceil(((a * m.buttonA.y) + (b * m.buttonB.y) - m.prize.y) / m.buttonA.y)
            }
        }

        if(m.prize.x == (a * m.buttonA.x) + (b * m.buttonB.x) && m.prize.y == (a * m.buttonA.y) + (b * m.buttonB.y)){
            console.log(i + 1, `a:${a}`, `b:${b}`, `cost:${b + (3*a)}`);
            total += (a * 3)+ b;
        } else {
            console.log(i +1, `nope`, a, b);
        }
    });
    console.log(`------`);
...
function parseButton(line, cost){
    var x = parseInt(line[1].split(',')[0].split('+')[1]);
    var y = parseInt(line[1].split(',')[1].split('+')[1]);
    return { x, y, cost}
}

function parsePrize(line){
    var x = parseInt(line[1].split(',')[0].split('=')[1]) + 10000000000000;
    var y = parseInt(line[1].split(',')[1].split('=')[1]) + 10000000000000;
    return { x, y}
}
